...
router.post('/', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO produtos (nome, preco, estoque, descricao, status) 
...
router.put('/:id/ativar', async (req, res) => {
  try {
    ...
    const { rowCount } = await pool.query(
      `UPDATE produtos SET status='ativo', data_alteracao=NOW() WHERE id=$1 AND status='excluido'`,
      [id]
    );routes
...
router.delete('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { rowCount } = await pool.query(
      `UPDATE produtos SET status='excluido', data_alteracao=NOW() WHERE id=$1`,
      [id]
    );
